...
class VideoListByViewView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        logger.info(f"Fetching all live videos sorted by video_views for language: {language}")
        videos = Video.objects.filter(
            status=VideoStatus.LIVE,
            frontend=frontend
        ).order_by('-video_views')[:50].values('youtube_id', 'youtube_title')
